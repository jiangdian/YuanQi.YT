...
public class VisionClass
{
    ...
    public void GrabImageVision(string device1, string device2, string trayCode1, string trayCode2, out HObject? ho_Image1, out HObject? ho_Image2, out string? image1Url, out string? image2Url)
    {
        try
        {
            ...
            double exposure1 = AdjustExposure(hv_AcqHandle1, Convert.ToDouble(configuration["leftRow1"]), Convert.ToDouble(configuration["leftColumn1"]),
                Convert.ToDouble(configuration["leftRow2"]), Convert.ToDouble(configuration["leftColumn2"]));
            HOperatorSet.SetFramegrabberParam(hv_AcqHandle1, "ExposureTime", exposure1);
            Thread.Sleep(500);
            HOperatorSet.GetFramegrabberParam(hv_AcqHandle1, "ExposureTime", out HTuple hv_Value1);
            HOperatorSet.OpenFramegrabber("GigEVision2", 0, 0, 0, 0, 0, 0, "progressive",
            -1, "default", -1, "false", "default", device2, 0, -1, out hv_AcqHandle2);
            double exposure2 = AdjustExposure(hv_AcqHandle2, Convert.ToDouble(configuration["rightRow1"]), Convert.ToDouble(configuration["rightColumn1"]),
                Convert.ToDouble(configuration["rightRow2"]), Convert.ToDouble(configuration["rightColumn2"]));
            HOperatorSet.SetFramegrabberParam(hv_AcqHandle2, "ExposureTime", exposure2);
            HOperatorSet.GetFramegrabberParam(hv_AcqHandle2, "ExposureTime", out HTuple hv_Value2);
            Thread.Sleep(500);
            ...
            string image1UrlWrite = @$"\\192.168.10.150\picture\vision\{DateTime.Now:yyddMMHHmmss}_{trayCode1}.jpg";
            HOperatorSet.WriteImage(ho_Image1, "jpg", 255, image1UrlWrite);
            foreach (string file in Directory.GetFiles(@"\\192.168.10.150\picture\vision"))
            {
            ...
            string image2UrlWrite = $@"\\192.168.10.150\picture\vision\{DateTime.Now:yyddMMHHmmss}_{trayCode2}.jpg";
            HOperatorSet.WriteImage(ho_Image2, "jpg", 255, image2UrlWrite);
        }
        catch (Exception ex)
        {
            ho_Image1 = null;
    ...
    private double AdjustExposure(HTuple hv_AcqHandle,double row1,double column1,double row2,double column2)
    {
        try
        {
            HOperatorSet.SetFramegrabberParam(hv_AcqHandle, "ExposureTime", 135000.0);
            HOperatorSet.GrabImage(out HObject ho_Image, hv_AcqHandle);
            HOperatorSet.Rgb1ToGray(ho_Image, out HObject ho_GrayImage);
            HOperatorSet.GenRectangle1(out HObject ho_Rectangle, row1, column1, row2, column2);
            HOperatorSet.Intensity(ho_Rectangle, ho_GrayImage, out HTuple hv_Mean, out _);
    ...
    {
        logger.LogInformation("开始识别条形码");
        try
        {
            HOperatorSet.CreateBarCodeModel(new HTuple(), new HTuple(), out HTuple hv_BarCodeHandle);
            HOperatorSet.FindBarCode(ho_Image, out _, hv_BarCodeHandle, "Code 128", out HTuple hv_DecodedDataStrings);
            if (hv_DecodedDataStrings != null)
            {
                for (int i = 0; i < hv_DecodedDataStrings.Length; i++)
                {
                    logger.LogInformation($"识别到条形码：{hv_DecodedDataStrings[i].S}");
    ...
    {
        logger.LogInformation("开始识别二维码");
        HOperatorSet.WriteImage(ho_Image, "jpg", 255, "./test.jpg");
        Thread.Sleep(500);
        ...
        try
        {
            ...
            if (hv_DecodedDataStrings != null)
            {
                for (int i = 0; i < hv_DecodedDataStrings.Length; i++)
                {
                    logger.LogInformation($"识别到二维码：{hv_DecodedDataStrings[i].S}");
                    materialNoList.Add(hv_DecodedDataStrings[i].S);
                }
            }
        }
